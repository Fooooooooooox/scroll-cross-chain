...
describe('WhitelistSale', function () {
  it('allow only whitelisted accounts to mint', async () => {
    ...
    const WhitelistSale = await ethers.getContractFactory('MyNFT')
    ...
    await expect(whitelistSale.mint(merkleProof)).to.eventually.be.rejected
    .and.have.property('message').that.includes('AlreadyClaimed');
    await expect(whitelistSale.connect(notWhitelisted[0]).mint(invalidMerkleProof)).to.eventually.be.rejected
    .and.have.property('message').that.includes('NotInMerkle');
  })
})